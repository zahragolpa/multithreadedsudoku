...
class MatrixSolution:
    ...
    def check_numbers_driver(self):
        ...
        for rows in self.rows:
            if not result:
                break
            t_row.append(threading.Thread(target=self.check_numbers, args=(rows,)))
        if result:
            for i in t_row:
                i.start()
            for i in t_row:
        ...
        for cols in self.cols:
            if not result:
                break
            t_col.append(threading.Thread(target=self.check_numbers, args=(cols,)))
        if result:
            for i in t_col:
                i.start()
            for i in t_col:
...
print("time consumed with 27 threads: " + str(time.time() - threadTime))
