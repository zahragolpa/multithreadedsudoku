...
class MatrixSolution:
    ...
    def check_numbers_driver(self):
        ...
        for rows in self.rows:
            if not result:
                break
            t_row.append(self.check_numbers(rows))
        # check columns
        for cols in self.cols:
            if not result:
                break
            t_col.append(self.check_numbers(cols))
        # check blocks
        block_line = []
        ...
        for line in block_line:
            if not result:
                break
            t_block.append(self.check_numbers(line))
...
print("time consumed with no threads: " + str(time.time() - initialTime))
